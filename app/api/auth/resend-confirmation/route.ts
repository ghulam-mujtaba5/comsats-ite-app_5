...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      return NextResponse.json({ error: 'Failed to send verification email', detail: txt }, { status: 500 })
    }
    return NextResponse.json({ message: 'Verification email sent' })
  } catch (e: any) {
    return NextResponse.json({ error: 'Network error when sending email' }, { status: 500 })
  }
}

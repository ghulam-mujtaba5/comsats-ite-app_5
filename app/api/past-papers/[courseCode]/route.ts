...
export async function GET(req: NextRequest, context: { params: Promise<{ courseCode: string }> }) {
  ...
  const mapPaperData = (papers: any[]) => {
    return papers.map((p) => ({
      ...
      examType: (p.type === 'Midterm' ? 'Mid-Term' : (p.type || p.exam_type || 'Mid-Term')),
  ...
  const courseWithPapers = {
    ...
    assignments: mapPaperData(papersData?.filter((p) => (p.type === 'Assignment' || p.exam_type === 'Assignment')) || []),
    quizzes: mapPaperData(papersData?.filter((p) => (p.type === 'Quiz' || p.exam_type === 'Quiz')) || []),
    midterms: mapPaperData(papersData?.filter((p) => (p.type === 'Mid-Term' || p.type === 'Midterm' || p.exam_type === 'Mid-Term' || p.exam_type === 'Midterm')) || []),
    finals: mapPaperData(papersData?.filter((p) => (p.type === 'Final' || p.exam_type === 'Final')) || []),
